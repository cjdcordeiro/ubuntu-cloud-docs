...
need to provision the EKS cluster with customized Ubuntu nodes.


With FIPS
^^^^^^^^^

When enabling FIPS, a reboot of the underlying node is required, which if done post-cluster creation, can lead to a defective cluster.
...
Create an ``eks-ubuntu-fips.json`` file with the following (replacing the
...
(replacing the "ami" field with the AMI ID from the previous step, and the
"ssh" field with a valid SSH key name):
...
to initialize the nodes. For further cluster customisation see `this <https://eksctl.io/>`_.
...
EKS AMIs and customizing it at deployment time, via cloud-init.
...
`LaunchTemplate <https://docs.aws.amazon.com/autoscaling/ec2/userguide/launch-templates.html>`_
on AWS.

LaunchTemplate user-data
************************

On the advanced section of your LaunchTemplate (user-data section), copy
...
To create a cluster with your custom LaunchTemplate, create a ``cluster.yaml``
with the following content (make sure the "launchTemplate" ID matches the one
from the template modified in the previous step):
...
This config file will allow you to create an EKS cluster using the LaunchTemplate
